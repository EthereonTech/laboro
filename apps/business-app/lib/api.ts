...
async function request<T>(
  ...
  if (res.status === 401 && !skipAuth) {
    ...
    if (newToken) {
      ...
      if (!retry.ok) {
        ...
        throw Object.assign(new Error(err?.error?.message ?? 'Erro'), { status: retry.status, code: err?.error?.code })
      }
  ...
  if (!res.ok) {
    ...
    throw Object.assign(new Error(err?.error?.message ?? 'Erro na requisição'), { status: res.status, code: err?.error?.code })
  }
